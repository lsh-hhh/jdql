...
def query_joy():
    resp = requests.get(
        assemble_request('ClientHandleService.execute', {"method": "queryJoyPage", "data": {"channel": 1}}))
...
def reward_task(task_id, active_type):
    resp = requests.get(assemble_request('ClientHandleService.execute', {
        ...
        "data": {"id": task_id, "channel": 1, "clientTime": + datetime.datetime.now() + 0.588,
...
def ap_do_task(taskType, taskId, channel, itemId):
    ...
    resp = requests.get(assemble_get_request('apDoTask',
                                             {"linkId": "toxw9c5sy9xllGBr3QFdYg", "taskType": taskType,
                                              "taskId": taskId,
...
def invite():
    ...
    requests.post(url_str, headers=headers, data=body_str)
